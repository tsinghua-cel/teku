...
class AttestationProductionDutyTest {
  ...
  // expectCreateAttestationData(validator2CommitteeIndex);
  ...
  //        duty.addValidator(
  //            validator1, validator1CommitteeIndex, validator1CommitteePosition, validator1Index,
  // 11);
  ...
  //            TYPE, SLOT, 1, Set.of(attestationData.getBeaconBlockRoot()), Optional.empty());
  //    verify(validatorLogger)
  //        .dutyFailed(TYPE, SLOT, Set.of(validator1.getPublicKey().toAbbreviatedString()),
  // failure);
  //    verifyNoMoreInteractions(validatorLogger);
  //
  //    verify(validatorDutyMetrics, times(2))
  ...
  //    verify(validatorLogger)
  //        .dutyFailed(
  //            TYPE, SLOT, Set.of(validator1.getPublicKey().toAbbreviatedString()),
  // signingFailure);
  //    verifyNoMoreInteractions(validatorLogger);
  //
  //    verify(validatorDutyMetrics)
  ...
  //
  //    // Should have only needed to create one unsigned attestation and reused it for each
  // validator
  ...
  //    try {
  //      result.get().report(TYPE, SLOT, validatorLogger);
